...
Scopes: `tweet.read tweet.write users.read media.write offline.access`. At connect time we read `GET /2/users/me?user.fields=subscription_type` to set the character limit (280, or 25,000 for `Premium` / `PremiumPlus`).
